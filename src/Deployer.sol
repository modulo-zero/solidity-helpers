...
abstract contract Deployer is Script {
    ...
    string internal deploymentEnvironment;
    ...
    function setUp() public virtual {
        ...
        (deploymentEnvironment,) = _getDeploymentContext();
        ...
        deploymentsDir = string.concat(root, "/deployments/", deploymentEnvironment);
    ...
    function _chainIsL1() internal view returns (bool) {
        return _compareStrings(_getLayer(), "l1");
    }

    function _chainIsL2() internal view returns (bool) {
    ...
    function computeAddress(address addr, uint64 offset) public view returns (address) {
        uint64 nonce = vm.getNonce(addr) + offset;
        bytes memory data;
    ...
    function _getEnvironment() internal view returns (string memory url, string memory layer, uint32 chainid) {
        ...
        if (bytes(_layer).length > 0) {
            network = res.readString(string.concat(".envs.", env, ".rpc.", _layer));
        } else {
            network = res.readString(string.concat(".envs.", env, ".rpc"));
        }
        url = res.readString(string.concat(".rpc.", network, ".url"));
        layer = res.readString(string.concat(".rpc.", network, ".layer"));
        chainid = uint32(res.readUint(string.concat(".rpc.", network, ".chainid")));
    }
